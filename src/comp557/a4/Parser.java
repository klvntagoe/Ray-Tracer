...
public class Parser {
	...
	public static Material createMaterial(Node dataNode) {
		...
		if( refAttr  != null ) {
	    	...
	    	Node reflectivenessAttr = dataNode.getAttributes().getNamedItem("reflectiveness");
	    	if ( reflectivenessAttr != null ) {
	    		float temp = Float.parseFloat( reflectivenessAttr.getNodeValue() );
	    		if (temp > 1) material.reflectiveness = 1.0f;
	    		else if (temp < 0) material.reflectiveness = 0.0f;
	    		else material.reflectiveness = temp;
	...
	public static Metaballs createMetaballs(Node dataNode){
	     Metaballs metaball = new Metaballs();
	     Node centers = dataNode.getAttributes().getNamedItem("centers");
	     if (centers != null){
            Scanner s = new Scanner(centers.getNodeValue());
            while(s.hasNextDouble()){
                double x = s.nextDouble();
                double y = s.nextDouble();
                double z = s.nextDouble();
                metaball.centers.add(new Point3d(x, y, z));
            }
        }
        Node threshhold = dataNode.getAttributes().getNamedItem("threshhold");
	     if (threshhold != null){
	         Scanner s = new Scanner(threshhold.getNodeValue());
	         metaball.threshhold = s.nextDouble();
        }
        metaball.material = parseMaterial(dataNode, "material");
	     metaball.setBoundingSpheres();
        return metaball;
    }
	/**
